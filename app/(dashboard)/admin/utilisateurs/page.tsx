...
//         if (value !== 'chef_produit') updated.pays   = ''
//       }
//       return updated
//     })
//   }
...
export default function UtilisateursPage() {
  ...
  const ouvrirDialogEdit = (user: User) => {
    ...
    setEditForm({
      ...
      password: '',                      // vide = pas de changement
      agence:   user.agence ?? '',
      pays:     user.pays   ?? '',        // pré-rempli avec le pays existant de l'utilisateur
    })
    setEditError('')
    setDialogEdit(true)
  }
    ...
    <Box sx={{ p: 1 }}>
      ...
      <Dialog open={dialogEdit} onClose={() => setDialogEdit(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent sx={{ pt: 2 }}>
          ...
          {/* Champ Pays — visible pour Conformité et Chef de produit, pré-rempli avec le pays existant */}
