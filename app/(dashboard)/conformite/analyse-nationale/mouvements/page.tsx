...
function AnalyseNationaleMouvementsContent() {
  ...
  useEffect(() => {
    ...
    const charger = async () => {
      ...
      setError('')
      try {
        const { data } = await archiveAgenceAPI.nationalParVille({ ville_id: Number(villeId) })
        setArchives(data)
      } catch {
...
export default function AnalyseNationaleMouvementsPage() {
  return (
    <Suspense fallback={<Box sx={{ p: 4, textAlign: 'center' }}><CircularProgress /></Box>}>
      <AnalyseNationaleMouvementsContent />
    </Suspense>
  )
}