...
import { 
  ...
  BadgeOutlined, RefreshOutlined, SummarizeOutlined,
  DownloadOutlined,
...
export default function ClientsAgencePage() {
  ...
  // L'isolation par pays est gérée côté backend (get_queryset) :
  // chaque rôle exporte automatiquement son propre périmètre.
  const handleExportCsv = async () => {
    setExporting(true)
    setError('')
    try {
      const res = await clientAPI.exporterCsv()

      // Récupère le nom de fichier suggéré par le backend si présent,
      // sinon retombe sur un nom par défaut.
      const disposition = res.headers?.['content-disposition'] as string | undefined
      const match = disposition?.match(/filename="?([^"]+)"?/)
      const filename = match?.[1] || 'clients.csv'
      ...
      console.error(err)
      setError("Impossible d'exporter la liste des clients.")
    } finally {
      setExporting(false)
    }
  }
              ...
              <TableRow key={c.id} hover>
                ...
                <TableCell>
                  <Box sx={{ display: 'flex', gap: 0.5 }}>
                    ...
                    <Tooltip title="Récapitulatif des transactions">
                      <IconButton
                        color="secondary"
                        onClick={() => router.push(`/conformite/clients/${c.id}/recap`)}
                      >
                        <SummarizeOutlined />
                      </IconButton>
                    </Tooltip>
